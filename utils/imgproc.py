...
def ycbcr2bgrTensor(image: torch.Tensor) -> torch.Tensor:
    """Implementation of ycbcr2bgr function in Matlab under Python language.
    ...
    #image_dtype = image.dtype
    image *= 255.
    t = torch.tensor([[0.00456621, 0.00456621, 0.00456621],
                              [0.00791071, -0.00153632, 0],
                              [0, -0.00318811, 0.00625893]]) * 255.0

    image = torch.matmul(image, t) + torch.tensor([-276.836, 135.576, -222.921])
    image /= 255.
    image = image.float()
